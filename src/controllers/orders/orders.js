  ...
  try {
    ...
    if (!user || !user.isActive) {
      throw new ApiError(
		...
		if (orderData.paymentOption === 'onDelivery') {
			if (orderData.paymentMethod !== 'Cash') {
				throw new ApiError(
					httpStatus.BAD_REQUEST,
					'Payment method must be Cash for on delivery payment option',
				);
			}
			...
			const emailPromises = Object.entries(cartItemsByVendor).map(async ([vendorId, items]) => {
				const vendor = await User.findById(vendorId)
				const vendorEmail = vendor.email
				const vendorGrandTotal = cartItems.reduce((total, cartItem) => {
					return total + cartItem.price * cartItem.quantity;
				}, 0);
	
				const emailContent = `You have a new order from ${user.username}. Please check your dashboard for details`
				return sendEmail({
					to: vendor.email,
					subject: 'New Order',
					text: emailContent,
    ...
    orderData.orderItems = await Promise.all(
      orderData.orderItems.map(async (item, index) => {
        const food = await Food.findById(item.foodId);

        console.log(
          `Food Price: ${
            food ? food.price : 'Food not found or price is undefined'
          }`,
        );
        if (!food || !food.price) {
          console.error(
            `Error: Food not found or price is undefined for order item ${
              index + 1
            }`,
          );
    ...
    if (orderData.paymentMethod === 'Card') {
      ...
      const paystackResponse = await initiatePaystackTransaction(
        user.email,
        totalAmount,
      );
...
const sendOrderNotificationAndPushNotification = async (orderData, userId) => {
  try {
    ...
    console.error(
      'Error sending order notification and push notification:',
      error.message,
    );
  }
};
...
const sendOrderNotificationEmail = async (
  ...
  try {
    ...
    const orderedItemsList = orderData.orderItems
      .map((item) => {
        // Map the additives for each item
        const additivesList = item.food.additives
          .map((additive) => `<li>${additive.name}</li>`)
          .join('');
        return `
							<li>
...
`;
    ...
    console.log(
      `Order notification email sent to restaurant: ${restaurantEmail}`,
    );
  } catch (error) {
...
const getUserOrders = Asyncly(async (req, res) => {
  ...
  const formattedOrders = orders.map((order) => {
    const formattedOrder = {
      ...
      drpOffLocation: order.deliveryAddress
